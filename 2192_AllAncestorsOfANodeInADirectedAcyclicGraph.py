...
class Solution:
        ...
        def findNode(cur, visit):
            ...
            for next_node in adj_map[cur]:
                if next_node not in visit:
                    findNode(next_node, visit)
        
        for node in range(N):
            visit = set()
            findNode(node, visit)
            visit.remove(node)
            res.append(sorted(list(visit)))
    ...
    def getAncestors(self, N: int, edges: list[list[int]]) -> list[list[int]]:
        ...
        def dfs(cur, parent):
            ...
            for next_node in adj_map[cur]:
                if len(res[next_node]) == 0 or res[next_node][-1] != parent:
                    res[next_node].append(parent)
    ...
    def getAncestors(self, N: int, edges: list[list[int]]) -> list[list[int]]:
        ...
        while q:
            ...
            for next_node in adj_map[cur]:
                ...
                res[next_node].add(cur)
                res[next_node] |= res[cur]
